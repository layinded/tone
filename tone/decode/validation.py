...
This module provides validation functions for strict mode decoding,
following SPEC Section 2 (Strict Mode).
"""

from typing import List

from tone.constants import COLON, LIST_ITEM_PREFIX
from tone.types import ArrayHeaderInfo, Delimiter, Depth, ResolvedDecodeOptions
...
def assert_expected_count(
    ...
    Args:
        ...
        item_type: The type of items being counted (e.g., 'list array items', 'tabular rows')
        options: Decode options
        
    Raises:
        RangeError: If counts don't match in strict mode
    """
...
def validate_no_extra_list_items(
    ...
    Raises:
        RangeError: If extra items are found
    """
    if cursor.at_end():
        return
...
def validate_no_extra_tabular_rows(
    ...
    Raises:
        RangeError: If extra rows are found
    """
    if cursor.at_end():
        return
    ...
    if (
        ...
        and is_data_row(next_line["content"], header["delimiter"])
    ):
...
def validate_no_blank_lines_in_range(
    ...
    """Validate that there are no blank lines within a specific line range and depth.
    
    Args:
        start_line: The starting line number (inclusive)
        end_line: The ending line number (inclusive)
        blank_lines: List of blank line information
    ...
    if len(blanks_in_range) > 0:
        first_blank = blanks_in_range[0]
        raise SyntaxError(
            f"Line {first_blank['line_number']}: Blank lines inside {context} are not allowed in strict mode"
        )


def is_data_row(content: str, delimiter: Delimiter) -> bool:
    """Check if a line represents a data row (as opposed to a key-value pair) in a tabular array.
